...
func (c *ExistenceConf) Conf(data *dpfm_api_input_reader.SDC, ssdc *dpfm_api_output_formatter.SDC, accepter []string, l *logger.Logger) (allExist bool, errs []error) {
	...
	for _, v := range *exConfMapper {
		...
		case "BusinessPartnerGeneral":
			wg.Add(1)
			go c.generalBPGeneralExistenceConf(v, data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
		case "Industry":
			wg.Add(1)
			go c.generalIndustryExistenceConf(v, data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
		case "Country":
			wg.Add(1)
		...
		case "Currency":
			wg.Add(1)
			go c.generalCurrencyExistenceConf(v, data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
		}
	}
