...
func (c *ExistenceConf) Conf(data *dpfm_api_input_reader.SDC, ssdc *dpfm_api_output_formatter.SDC, l *logger.Logger) (allExist bool, errs []error) {
	...
	wg.Add(5)
	go c.businessPartnerExistenceConf(data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
	go c.industryExistenceConf(data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
	go c.countryExistenceConf(data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
	go c.languageExistenceConf(data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
	go c.currencyExistenceConf(data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
