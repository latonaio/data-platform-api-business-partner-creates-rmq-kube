...
func (c *ExistenceConf) countryExistenceConf(input *dpfm_api_input_reader.SDC, existenceMap *[]bool, exconfErrMsg *string, errs *[]error, mtx *sync.Mutex, wg *sync.WaitGroup, log *logger.Logger) {
	defer wg.Done()
	if input.General.Country != nil {
		if len(*input.General.Country) == 0 {
			*exconfErrMsg = "cannot specify null keys"
			return
		}
	}
	res, err := c.countryExistenceConfRequest(*input.General.Currency, input, existenceMap, mtx, log)
	if err != nil {
		mtx.Lock()
